    ...
    <table class="table">
      ...
      <tbody v-if="platforms.data.length" class="tbody">
        <tr v-for="(platform, index) in platforms.data" :key="index">
          ...
          <td>
            <span v-if="platform.status" class="badge"
              :class="platform.status == 'authenticated' ? 'badge-success' : 'badge-danger'">
              {{ platform.status }}
            </span>
                ...
                <div class="dropdown-content w-56">
                  <ul class="dropdown-list">
                    <li class="dropdown-list-item">
                      <a class="dropdown-link" @click="deleteRow(route('admin.logs.platforms.destroy', platform))">
