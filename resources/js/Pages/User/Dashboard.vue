  ...
  <div class="grid grid-cols-12 gap-4">
    <div class="col-span-full space-y-5 xl:col-span-7">
      ...
      <div class="card rounded-2xl p-5" v-else>
        <div class="flex flex-wrap justify-between gap-2">
          ...
          <select v-model="filterForm.customers" @change="
            ...
            <option :value="item" v-for="item in dateFilterQueries" :key="item" :selected="filterForm.sales === item">
              {{ item }}
            </option>
          </select>
        </div>
        <VueApexCharts v-if="$el" height="400" :options="growthChart.chartOptions" :series="growthChart.series" />
      </div>
      ...
      <div class="card rounded-2xl p-5" v-else>
        <div class="flex flex-wrap justify-between gap-2">
          ...
          <select v-model="filterForm.campaigns" @change="
        ...
        <VueApexCharts v-if="$el" height="400" :options="campaignChart.chartOptions" :series="campaignChart.series" />
      </div>
      <ChartSkeleton class="mt-5" v-if="analytics.messages.loading" />
      <div class="card rounded-2xl p-5">
        <div class="flex flex-wrap justify-between gap-2">
          <h6>{{ trans('Message Statistics') }}</h6>
          <select v-model="filterForm.messages" @change="
            dashboardStore.fetchAnalytics({ type: 'messages', filter: filterForm.messages })
            " class="select select-xl w-full capitalize md:w-40">
            <option value="" selected>{{ trans('Filter By') }}</option>
            <option :value="item" v-for="item in dateFilterQueries" :key="item"
              :selected="filterForm.messages === item">
              {{ item }}
            </option>
          </select>
        </div>
        <VueApexCharts v-if="$el" height="400" :options="messageChart.chartOptions" :series="messageChart.series" />
      </div>
    </div>
    <div class="col-span-full space-y-4 xl:col-span-5">
      ...
      <div class="col-span-1 lg:col-span-4" v-else>
        <div class="card rounded-2xl rounded-b-none rounded-t-2xl">
          ...
          <div class="flex justify-around">
            <div v-for="item in currentWeekDays" :key="item" class="flex flex-col items-center justify-center">
              <!-- if day is today -->
              <p class="my-2 flex h-10 w-10 items-center justify-center rounded-full" :class="{
        ...
        <div class="card mt-3 rounded-2xl rounded-b-2xl rounded-t-none p-4">
          <div v-if="analytics.schedule_campaigns.data.length > 0" class="grid grid-cols-2 gap-3">
            <div v-for="campaign in analytics.schedule_campaigns.data" :key="campaign.id"
              ...
              <div>
                ...
                <p class="-mt-0.5 flex items-center gap-0.5 text-[11px] text-gray-500 dark:text-gray-400">
      ...
      <div class="card card-body flex items-center justify-between rounded-2xl" v-if="!loading">
        ...
        <div class="flex min-h-16 min-w-16 items-center justify-center rounded-full bg-primary-600/60">
      ...
      <div class="card card-body flex items-center justify-between rounded-2xl" v-if="!loading">
        <div class="flex flex-col">
          ...
          <a class="btn btn-soft-primary mt-6 max-w-max" :href="route('user.whatsapp-web.platforms.index')">
            <span>
              {{ trans('Setup Account') }}
            </span>
      ...
      <div class="card card-body flex items-center justify-between rounded-2xl" v-if="!loading">
        <div class="flex flex-col">
          ...
          <a class="btn btn-soft-primary mt-6 max-w-max" href="/user/whatsapp/platforms">
            <span>
              {{ trans('Setup Account') }}
            </span>
            <Icon icon="solar:arrow-right-broken" class="mt-1 size-4" />
      ...
      <div class="card flex h-56 flex-col items-center justify-center rounded-2xl" v-else>
        <VueApexCharts v-if="$el" width="100%" height="380" type="radialBar" :options="storageChart.chartOptions"
      ...
      <div class="card card-body rounded-2xl">
        ...
        <div class="flex items-center justify-center pt-5">
          <VueApexCharts v-if="$el" height="160" type="donut" :options="pieChart.chartOptions"
            :series="pieChart.series" />
        </div>

        <div class="flex h-3 rounded-md pb-5 pt-8">
          <div v-for="(item, index) in calculatePlatformPercentages" :key="item.name"
            class="relative h-4 rounded hover:bg-opacity-20"
            :style="`width: ${item.value}%; background: ${chartColors[index]}`">
            <span class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-xs text-dark-300">
              {{ item.value }}%
            </span>
      ...
      <div class="card card-body relative space-y-2 overflow-hidden rounded-2xl">
        <div class="card absolute inset-0 z-10 flex flex-col items-center justify-center rounded-md"
        ...
        <div class="flex items-stretch gap-2">
          <input type="text" class="input" placeholder="Auth key"
            :value="isCurrentWorkspaceOwner ? authUser.authkey : ''" />
          <button type="button" @click="copyToClipboard(isCurrentWorkspaceOwner ? authUser.authkey : '')"
            class="btn btn-soft-primary">
            <Icon icon="bx:copy" />
          </button>
        </div>

        <Link as="button" :href="route('user.account-settings.regenerate-key')" method="post"
          class="btn btn-soft-primary w-full" preserve-scroll>
