  ...
  <div
    ...
    <div class="relative mb-3">
      <div class="mb-2 flex items-center gap-2 text-nowrap">
        <select class="select" v-model="chatStore.searchForm.badge_id" @change="getFilteredConversations">
      ...
      <form @submit.prevent="getFilteredConversations"
        ...
        <div class="flex h-full items-center px-3">
          <Icon icon="fe:search" class="text-lg text-slate-400 group-focus-within:text-primary-500" />
        </div>
        <input
          class="h-full w-full border-transparent bg-transparent px-0 text-sm placeholder-slate-400 placeholder:text-sm focus:border-transparent focus:outline-none focus:ring-0"
          type="text" v-model="chatStore.searchForm.customer_name" @keyup="getFilteredConversations"
          placeholder="Search" />
      </form>

      <!-- Chat Sidebar Close Button Starts -->
      <button id="chat-btn-hide-sidebar" type="button" v-if="chatStore.leftSidebar.isOpen"
    ...
    <div class="styled-scrollbar h-[88%] overflow-y-auto">
      <ul id="chat-list" ref="scrollContainerRef">
        <li v-for="chat in chatStore.filteredConversations" class="group" :class="{
          active: chatStore.activeConversationId == chat.id
        }" :key="chat.id">
          <Link :href="getActiveModuleConversationLink(chat)"
          ...
          <div class="avatar avatar-circle shrink-0">
            <img class="avatar-img relative" v-if="chat.customer?.picture" :src="chat.customer?.picture" />
            <div class="absolute -right-2 bottom-0 rounded-full bg-white p-1">
              <Icon :icon="`ri:${chat.module}-fill`" :class="{
          ...
          <div class="ml-4 mr-2 flex-grow overflow-x-hidden">
            <p class="whitespace-nowrap text-sm text-slate-800 dark:text-slate-100"
        ...
        ">
          ...
          <div v-else-if="chatStore.filteredConversations.length == 0" class="text-center text-sm text-slate-400">
            {{ trans('No results found') }}
          </div>
        </li>
        <li class="py-1">
          <IntersectionObserver :afterIntersection="() => chatStore.loadMoreConversations()"
            :loader="chatStore.loading.conversations" />
        </li>
      </ul>
    </div>
  </div>
  <div @click="chatStore.toggleLeftSidebar" v-if="chatStore.leftSidebar.isOpen"
