...
const mdGreater = breakpoints.greater('md')
...
const attachMedia = (mediaType) => {
  assetStore.openModal({
    ...
    onOpen: () => {
      chatStore.assetPopup = false
      chatStore.inputMessage.type = mediaType
    },
    onSelect: (asset) => { },
    onSubmit: (assets, state) => {
      chatStore.inputMessage.attachments = assets.map((asset) => asset.path)
      chatStore.inputMessage.caption = state.caption

      if (chatStore.inputMessage.attachments?.length === 0) {
...
const sendTextMessage = () => {
  ...
  if (chatStore.inputMessage.message.length === 0) {
...
<template>
  <div
    class="relative flex h-[calc(100vh-11rem)] w-full flex-col overflow-hidden rounded-md bg-white shadow transition-all dark:bg-dark-800">
    <Header />
    <div class="styled-scrollbar whatsapp-chat-body relative h-[90%] overflow-auto px-4 pb-8"
      :class="`${$page.props.activeModule}-chat-body`">
      <ul class="space-y-3" id="scrollContainerRef" v-if="chatStore.activeConversationMessages?.length">
        <li v-if="chatStore.activeConversation.no_more_messages">
          <p class="mt-4 text-center text-sm font-semibold italic text-gray-300">
            {{ trans('No more messages') }}
          </p>
        </li>
        <li class="text-center" v-else>
          <IntersectionObserver :loader="chatStore.loading.messages"
            :afterIntersection="() => chatStore.loadMoreMessages()" />
        </li>

        <li v-for="(message, index) in chatStore.activeConversationMessages" class="group" :key="index"
          ...
          <div class="flex gap-x-3 group-[.pr]:flex-row-reverse">
            <div v-if="message.direction == 'in'" class="avatar avatar-circle avatar-sm shrink-0">
              <img :src="chatStore.activeConversation?.customer?.picture" class="avatar-img" alt="profile-img" />
            </div>
            <div class="flex max-w-sm flex-col items-start group-[.pr]:items-end">
              <p v-if="message.meta?.context?.id"
              ...
              <div class="flex items-center">
                ...
                <button v-if="message.direction == 'in'" @click="chatStore.setReplying(message)"
              ...
              <div class="mb-2 space-x-2">
                ...
                <span v-if="message.direction == 'out'" class="font-bolder ml-1 text-slate-300" :title="message.status == 'failed'
                  ...
                  <i v-else-if="message.status == 'read'" class="bx bx-check-double text-green-600"></i>
                  <i v-else-if="message.status == 'failed'" class="bx bx-info-circle text-red-600"></i>
                </span>
              </div>
            </div>
          </div>
    ...
    <div class="relative rounded-b-primary border-t bg-white dark:border-dark-700 dark:bg-dark-800">
      <div class="alert alert-danger flex justify-between p-2"
        v-if="chatStore.activeConversation?.meta?.is_24h_passed == true">
        <p>
          {{
      ...
      <template v-else>
        ...
        <VoiceRecorder v-if="chatStore.inputMessage.type === 'voice'" :store="chatStore" />
        <QuickReplyPreview v-show="chatStore.quickReplySuggestionItems.length > 0" />
        <form v-if="chatStore.inputMessage.type != 'voice'" @submit.prevent="sendTextMessage"
          class="flex items-center rounded-primary shadow">
          <CustomTextarea id="inputMessageField" v-model="chatStore.inputMessage.message" :placeholder="`Type message...${mdGreater ? 'enter to send, shift+enter to next line' : ''
            }`" :disabled="chatStore.loading.sendingMessage" @keydown.tab.prevent="chatStore.selectQuickReply"
            @keyup.down="chatStore.onArrowDown" @keydown.enter.exact.prevent="sendTextMessage"
            @keyup.up="chatStore.onArrowUp" autofocus />

          <div class="flex items-center gap-x-4 px-4">
            <button v-if="chatStore.features?.voice_message" type="button"
              @click="chatStore.inputMessage.type = 'voice'" title="Voice"
            ...
            <div class="relative">
              <button id="btn-upload-media" @click="chatStore.assetPopup = !chatStore.assetPopup" type="button"
                title="Media"
                class="text-slate-500 hover:text-primary-500 focus:text-primary-500 dark:text-slate-400 dark:hover:text-primary-500 dark:focus:text-primary-500">
                <Icon icon="bx:link-alt"></Icon>
              </button>
              <div class="fixed inset-0 bg-transparent" v-if="chatStore.assetPopup"
                @click="chatStore.assetPopup = false"></div>
              <transition name="fade">
                <div v-if="chatStore.assetPopup"
                  class="absolute bottom-full right-0 mb-2 space-y-2 rounded-md bg-dark-900 p-3">
                  <button @click="() => attachMedia('document')" type="button" class="btn btn-dark btn-xs w-full">
                    <Icon icon="bx:file"></Icon>
                    <span>{{ trans('Documents') }}</span>
                  </button>

                  <button @click="attachMedia('image')" type="button" class="btn btn-dark btn-xs w-full">
                    <Icon icon="bx:image"></Icon>
                    <span>{{ trans('Image') }}</span>
                  </button>

                  <button @click="() => attachMedia('video')" type="button" class="btn btn-dark btn-xs w-full">
                    <Icon icon="bx:video"></Icon>
                    <span>{{ trans('Video') }}</span>
                  </button>

                  <button @click="() => attachMedia('audio')" type="button" class="btn btn-dark btn-xs w-full">
            ...
            <button type="button" @click="chatStore.quickReplyModalOpen" title="Quick Reply"
              class="text-slate-500 hover:text-primary-500 focus:text-primary-500 dark:text-slate-400 dark:hover:text-primary-500 dark:focus:text-primary-500">
              <i class="bx bxs-layer"></i>
            </button>
            <button type="button" @click="modalStore.open('templateModal')" title="Templates"
              class="text-slate-500 hover:text-primary-500 focus:text-primary-500 dark:text-slate-400 dark:hover:text-primary-500 dark:focus:text-primary-500">
