...
const sendMessage = () => {
  ...
  chatStore.inputMessage.attachments = [audio.value.blob]
...
<template>
  <div class="alert alert-danger flex h-16 items-center justify-between px-6" v-if="!isDeviceSupportAudioInput">
    <p class="">
      {{
  ...
  <div v-else class="flex h-16 items-center justify-between rounded-md bg-gray-50 px-4 shadow-sm dark:bg-dark-800">
    <button type="button" @click="closeRecording"
    ...
    <div class="mr-3 flex flex-1 items-center justify-end">
      ...
      <div class="rounded-full bg-slate-200 dark:bg-dark-600">
        <audio v-if="audio.url && !audio.isRecording" :src="audio.url" controls
          class="h-10 w-64 opacity-60 dark:opacity-50"></audio>
      </div>
    </div>

    <div class="mr-2 flex items-center gap-3">
      <button v-if="audio.url" type="button" @click="deleteRecord"
        class="flex h-8 w-8 items-center justify-center rounded-full transition-colors hover:bg-gray-200 dark:hover:bg-dark-700">
        <Icon icon="bx:trash" class="text-red-500 dark:text-red-400" />
      </button>

      <button type="button" v-if="!audio.url && !audio.isRecording" @click="startRecording"
      ...
      <button type="button" v-else-if="!audio.url && audio.isRecording" @click="stopRecording"
      ...
      <button v-if="audio.url && !audio.isRecording" @click="sendMessage" :disabled="chatStore.loading.sendingMessage"
