...
<template>
  ...
  <div
    class="card absolute overflow-y-auto right-0 z-20 ml-2 h-[calc(100vh-11rem)] w-10/12 rounded-md p-1 transition-all duration-300 lg:w-3/12"
    :class="[chatStore.rightSidebar.isOpen ? '-translate-x-0' : 'translate-x-[125%]']">
    <button type="button" @click="chatStore.toggleRightSidebar"
    ...
    <div class="mt-3 flex flex-col items-center justify-center">
      <button class="group relative h-14 w-14" type="button" @click="profilePictureRef.click()">
        <img class="h-full w-full rounded-full border object-contain object-center"
          :src="chatStore.activeConversation?.customer?.picture" alt="profile-img" />
        <div
        ...
        <input ref="profilePictureRef" class="hidden" type="file" @change="uploadProfilePicture"
          accept="image/jpg, image/jpeg, image/png" placeholder="change" />
      </button>
      ...
      <div class="mt-2">
        <p class="cursor-pointer text-sm font-semibold text-primary-600" @click="openMessageSearchModal">
    ...
    <div class="mt-4 space-y-4 p-4">
      <div class="flex items-center gap-2">
        ...
        <template v-if="chatStore.activeConversation?.badge_id">
          ...
          <Icon icon="bx:trash" @click="removeChatBadge(chatStore.activeConversationId)"
            class="cursor-pointer text-danger-700" />
        </template>
        <Icon v-else icon="bx:plus" @click="modalStore.open('addBadgeModal')" class="cursor-pointer" />
      </div>
      <div class="flex items-center gap-2">
      ...
      <div class="flex items-center gap-2">
        <Icon icon="bx:user" class="mt-px" />
        <span class="text-sm">{{ trans('Customer ID') }}</span>
        <span class="text-sm font-bold"> {{ chatStore.activeConversation?.customer?.uuid }}</span>
      </div>

      <div class="flex items-center gap-2">
        <Icon icon="bx:bxs-school" class="mt-1" />
        <span class="text-sm">{{ trans('Platform') }}</span>
      ...
      <div>
        <div class="flex justify-between">
          <p>{{ trans('Note') }}</p>
          <Icon v-if="isNoteEditing" icon="bx:x" class="cursor-pointer text-xl" @click="isNoteEditing = false" />
          <Icon v-else icon="bx:edit" class="cursor-pointer" @click="isNoteEditing = true" />
        </div>

        <div v-if="isNoteEditing">
          <textarea class="textarea" id="note" v-model="customerDetailsForm.note" placeholder="Enter note here">
    </textarea>
          <SpinnerBtn :processing="customerDetailsForm.processing" @click="updateNote" :btn-text="trans('Save Note')"
            class="w-full" />
        </div>
        ...
        <div v-else>
          ...
          <div v-else>
            <div class="text-center">
              <p class="small mb-2 italic text-gray-500">
                <span>
                  {{ trans('Note not set yet') }}
                </span> &nbsp;
                <span class="text-primary-600 cursor-pointer" @click="isNoteEditing = true">
                  {{ trans('Add Note') }}
                </span>
  ...
  <div @click="chatStore.toggleRightSidebar" v-if="chatStore.rightSidebar.isOpen"
    class="absolute inset-0 z-0 h-full w-full bg-black/20 transition-colors duration-300 ease-in-out"></div>
</template>
