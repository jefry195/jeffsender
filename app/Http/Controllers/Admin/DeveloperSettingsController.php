...
class DeveloperSettingsController extends Controller
{
    ...
    public function show($id)
    {
        ...
        if ($id == 'app-settings') {
            ...
            $HOST = data_get($waServerEnv, 'HOST');
    ...
    public function update(Request $request, $id)
    {
        if ($id == 'app-settings') {
            ...
            $this->editWaServerEnv('HOST', $request->get('HOST'));
            $this->editEnv('WHATSAPP_WEB_API_BASE_URL', 'http://'.$request->get('HOST').':'.$request->get('PORT'));
            $this->editWaServerEnv('BASE_URL', url('/'));
            $this->editWaServerEnv('SITE_KEY', env('SITE_KEY'));
            $this->editWaServerEnv('MAX_RETRIES', env('MAX_RETRIES',5));
            $this->editWaServerEnv('RECONNECT_INTERVAL', env('RECONNECT_INTERVAL',5000));
            $this->editWaServerEnv('APP_WEBHOOK_ALLOWED_EVENTS', 'ALL');
            $this->editWaServerEnv('APP_WEBHOOK_FILE_IN_BASE64', 'false');
            $this->editWaServerEnv('STORE_TYPE', 'database');
