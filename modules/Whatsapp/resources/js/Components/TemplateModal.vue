...
<template>
  <Modal :header-state="true" header-title="Message Template" state="templateModal" state-key="templateModal"
          ...
          <div class="col-span-full w-full lg:col-span-2">
            <div class="group mb-5 flex items-center rounded-lg dark:bg-slate-700">
              <select v-model="selectedTemplateId" class="select w-full">
                ...
                <option v-for="item in chatStore.getActiveModuleTemplates" :key="item.id" :value="item.id">
                  {{ item.name }}
                </option>
              </select>
            </div>
            <div class="styled-scrollbar h-[calc(100vh-10rem)] overflow-y-auto">
              <InteractiveCreateForm v-if="selectedTemplate?.type == 'interactive'" :meta="selectedTemplate.meta"
                :preview="false" />
              <MessageForm v-else-if="selectedTemplate?.type" :type="selectedTemplate.type"
                :body="selectedTemplate.meta" />
            </div>
          </div>
          <div class="col-span-full w-full rounded-md bg-slate-200 p-2 dark:bg-dark-800 lg:col-span-1">
            <p class="my-4">{{ trans('Preview') }}</p>
            <div class="card-body whatsapp-chat-body">
              <InteractivePreview v-if="selectedTemplate?.type == 'interactive'" :components="selectedTemplate.meta" />

              <MessagePreview v-else-if="selectedTemplate?.type"
                :message="{ type: selectedTemplate.type, body: selectedTemplate.meta }" />

            </div>
            <div class="card-footer">
              <SpinnerBtn v-if="selectedTemplate?.id" @click="sendTemplateMessage" type="button"
                :processing="chatStore.loading.sendingMessage" class="btn btn-primary float-right mt-2"
                btn-text="Send" />
            </div>
          </div>
        </div>
