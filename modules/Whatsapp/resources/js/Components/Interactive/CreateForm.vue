...
<template>
  <div v-for="(component, key) in props.meta" :key="key" class="col-span-full w-full md:col-span-2">
    <div v-if="!['type', 'schedule_timezone', 'schedule_timestamp'].includes(key)"
      ...
      <div class="space-y-2 p-2" v-if="key == 'header'">
        ...
        <div class="align-end flex flex-col justify-between gap-1">
          ...
          <div v-if="component.type == 'text'">
            <InputField v-model="component.text" placeholder="Enter text value"
              :validation-message="errors[`meta.header.text`]" max-length="60" />
      ...
      <div class="space-y-2 p-2" v-if="key == 'footer'">
        <div class="align-end flex flex-col justify-between gap-1">
          <InputField v-model="component.text" max-length="60" placeholder="Enter text value"
      ...
      <div v-if="key === 'action'" class="mt-3 space-y-3 p-2">
        <div v-if="meta.type == 'button'" class="p-1">
          <button type="button" v-if="meta.action.buttons.length < 3" @click="addNewReplyButton()"
          ...
          <div v-for="(button, btIndex) in component.buttons" :key="btIndex" class="mb-2">
            ...
            <div class="flex gap-1">
              <input type="text" v-model="button.reply.title" class="input" placeholder="enter button text" />
        ...
        <div v-if="meta.type == 'product'">
          ...
          <div class="grid grid-cols-2 items-center gap-3 pb-3">
            ...
            <InputField v-model="component.product_retailer_id" placeholder="product-SKU-in-catalog" />
            <div class="col-span-full text-sm">
              {{
        ...
        <div v-if="meta.type == 'list'">
          <div class="w-full">
            <InputField label="List Button Text" v-model="component.button" placeholder="Enter action title" />
          </div>

          <div class="mt-3 flex flex-col gap-2">
            <div class="flex items-center justify-between">
              <label class="label capitalize">{{ trans('Section List') }}</label>
              <button type="button" @click="addNewItem('section', component)" class="btn btn-primary btn-sm">
                <i class="bx bx-plus"></i>
                {{ trans('Add Section') }}
              </button>
            </div>
            <div v-for="(section, sectionIndex) in component.sections" :key="sectionIndex"
              class="space-y-1 rounded-lg border-t-2 border-t-purple-600 p-2">
              <div class="flex items-center justify-between">
                <label class="label mb-1">{{ trans('') }} {{ trans('Section') }} {{ sectionIndex + 1 }} Title
                </label>
                <div v-if="sectionIndex != 0" @click="removeItem('section', component, sectionIndex)"
              ...
              <div class="flex flex-col gap-3">
                <div v-for="(row, rowIndex) in section.rows" :key="rowIndex" class="flex flex-col gap-3">
                  <div>
                    <div class="grid grid-cols-2 gap-3">
                      <div class="w-full">
                        <InputField :label="`Item ${rowIndex + 1} Title`" v-model="row.title"
                          placeholder="Enter title" />
                      </div>
                      <div class="w-full">
                        <div class="flex items-center justify-between">
                          <label class="label mb-1 capitalize">{{ trans('Item') }}
                            {{ rowIndex + 1 }}
                            {{ trans('Description') }}</label>
                          <div v-if="rowIndex != 0" @click="removeItem('row', section, rowIndex)"
                            class="cursor-pointer text-xl text-red-600 hover:text-red-700">
                            <i class="bx bx-trash"></i>
                          </div>
                        </div>
                    ...
                    <div v-if="rowIndex + 1 === section.rows.length" class="mt-2 flex justify-end">
                      <button type="button" @click="addNewItem('row', section)" class="btn btn-primary btn-xs">
                        <i class="bx bx-plus"></i>
                        {{ trans('Add Row') }}
                      </button>
                    </div>
        ...
        <div v-if="meta.type == 'product_list'">
          <div class="w-full">
            ...
            <input type="text" class="input" placeholder="Example: t-56fdofer" v-model="component.catalog_id" />
          </div>
          <div>
            <div class="flex flex-col gap-3">
              ...
              <div v-for="(section, sectionIndex) in component.sections" :key="sectionIndex"
                ...
                <div class="mb-6">
                  <div class="flex items-center justify-between">
                    <label class="label mb-1">{{ trans('') }} {{ trans('Section') }} {{ sectionIndex + 1 }}
                      {{ trans('Title') }}
                    </label>
                    <div v-if="sectionIndex != 0" @click="removeItem('section', component, sectionIndex)"
                      class="cursor-pointer text-xl text-red-600 hover:text-red-700">
                  ...
                  <input type="text" class="input" placeholder="product-SKU-in-catalog" v-model="section.title" />
                </div>
                <div class="flex flex-col gap-4">
                  <div v-for="(row, rowIndex) in section.product_items" :key="rowIndex" class="flex flex-col gap-3">
                    <div>
                      <div class="w-full">
                        <div class="flex items-center justify-between">
                          <label class="label mb-1 capitalize">{{ trans('Product Retailer Id') }}
                            {{ rowIndex + 1 }}
                          </label>
                          <div v-if="rowIndex != 0" @click="removeItem('product_items', section, rowIndex)"
                            class="cursor-pointer text-xl text-red-600 hover:text-red-700">
                            <i class="bx bx-trash"></i>
                          </div>
                        </div>
                        <input type="text" class="input" placeholder="product-SKU-in-catalog"
                          v-model="row.product_retailer_id" />
                      </div>
                      <div v-if="rowIndex + 1 === section.product_items.length" class="mt-3 flex justify-end">
                        <button @click="addNewItem('product_items', section)" type="button"
