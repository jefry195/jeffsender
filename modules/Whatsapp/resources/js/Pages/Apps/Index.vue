...
const props = defineProps(['apps', 'platforms'])
...
<template>
  <Modal state="appModal" :header-state="true" header-title="Create New App">
    <form @submit.prevent="submit" class="space-y-2">
      ...
      <div>
        <label class="label mb-1">{{ trans('App Name') }}</label>
        <input type="text" class="input" v-model="form.name" />
        <small class="text-red-600" v-if="form.errors.name">{{ form.errors.name }}</small>
      </div>
      <div>
        <label class="label mb-1">{{ trans('Web Link') }}</label>
          ...
          <FloatingDropdown btn-type="icon" icon-name="bx:dots-vertical-rounded">
            <ul class="dropdown-list">
              <li class="dropdown-list-item">
                <button @click="openInfoModal(app)" class="dropdown-link gap-2">
                  <Icon icon="bx:detail" />
                  {{ trans('Info') }}
                </button>
              </li>
