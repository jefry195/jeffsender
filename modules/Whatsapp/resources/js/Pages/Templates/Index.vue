    ...
    <table class="table">
      ...
      <tbody v-if="templates.data.length" class="tbody">
        <tr v-for="(template, index) in templates.data" :key="index">
          <td>
            <Link :href="route('user.whatsapp.templates.show', template)">
            {{ template.name }}
            </Link>
          </td>
          <td>
          ...
          <td>
            <span class="badge uppercase" :class="['APPROVED', 'active'].includes(template.status) ? 'badge-success' : 'badge-warning'
              ">
                  ...
                  <ul class="dropdown-list">
                    <template v-if="template.type !== 'template'">
                      <li class="dropdown-list-item">
                        <Link class="dropdown-link" :href="route('user.whatsapp.templates.copy', template.id)">
                        <Icon class="h-5 text-3xl text-slate-400" icon="bx:copy" />
                        {{ trans('Copy') }}
                        </Link>
                      </li>
                      <li class="dropdown-list-item">
                        <Link class="dropdown-link" :href="route('user.whatsapp.templates.edit', template.id)">
