  ...
  <div class="grid grid-cols-1 gap-y-4 md:gap-4 lg:grid-cols-3">
      ...
      <table class="table">
        ...
        <tbody class="tbody" v-if="logs.data.length">
          <tr v-for="(log, index) in logs.data" :key="index">
            <td>
              <a :href="`https://wa.me/${log.meta?.phone}`" target="_blank" rel="noopener noreferrer"
                class="hover:underline">
                {{ log.meta?.phone }}
              </a>
            </td>
            <td>
              <i v-if="log.failed_at" class="bx"
                :class="log.failed_at ? 'bxs-info-circle text-red-500' : 'bx-info-circle'"
                :title="log.meta.errors?.[0]?.error_data?.details ?? log.failed_at"></i>
              <i v-else class="bx" :class="log.send_at ? 'bxs-check-circle text-green-500' : 'bx-circle'"
                :title="log.send_at"></i>
            </td>
            <td>
              <i class="bx" :class="log.delivered_at ? 'bxs-check-circle text-green-500' : 'bx-circle'"
                :title="log.delivered_at"></i>
            </td>
            <td>
              <div class="text-end">
                <i class="bx" :class="log.read_at ? 'bxs-check-circle text-green-500' : 'bx-circle'"
                  :title="log.read_at"></i>
              </div>
            </td>
          </tr>
    ...
    <div class="col-span-full space-y-4 lg:col-span-1">
      <div class="card card-body flex h-56 flex-col items-center justify-center rounded-2xl">
        <VueApexCharts height="200" :options="campaignPieChart.chartOptions" :series="campaignPieChart.series" />
      </div>
          ...
          <div v-for="(log, i) in logsWithPercentage" :key="i">
            ...
            <div class="h-1.5 w-full rounded-lg bg-gray-200">
              <div class="h-full rounded-lg transition-all"
                :style="{ width: log.percentage + '%', backgroundColor: chartColors[i] }"></div>
            </div>
          </div>
        </div>
      </div>
