...
export const useChatStore = defineStore('chatStore', () => {
  ...
  //------------------ conversations
  const conversationPagination = ref({ limit: 10 })
  ...
  const loadMoreMessages = (chat, reset = false) => {
    ...
    if (Object(chat).hasOwnProperty('cursor') && !reset) {
      if (chat.cursor === null) {
    ...
    axios
      .get(apiUrl(`${sessionId}/chats/${conversationId}`), {
        params: {
          ...
          cursorId: chat.cursor
        }
      })
      .then(function (res) {
        let messages = res.data?.data?.messages || []
        if (messages.length > 0) {
          let allMessages = [...messages, ...activeConversation.value.messages]
        ...
        let hasMore = res.data?.data?.hasMore
        if (hasMore) {
          chat.cursor = res.data?.data?.nextCursor?.id || null
        } else {
          chat.cursor = null
        }
  ...
  const getMedia = async (message) => {
    ...
    let sessionId = activeConversation.value.sessionId

    try {
      const response = await axios.post(apiUrl(`${sessionId}/messages/download`), message.key, {
        responseType: 'blob'
      })
  ...
  const handleLiveChatNotifyEvent = (payload) => {
    ...
    } else if (payload.event == 'CHATS_UPSERT') {
      handleChatsUpsertEvent(payload)
    } else if (payload.event == 'CHATS_UPDATE') {
  ...
  const handleMessagesUpsertEvent = (payload) => {
    ...
    let { remoteJid, remoteJidAlt } = message.key || {}
    if (!remoteJid) return
    ...
    // Find chat once instead of iterating with side effects
    const chat = conversations.value.find(
      (c) => c.id === remoteJid && c.sessionId === sessionId
    )

    if (!chat) {
      console.log('Chat not found', { remoteJid, sessionId, payload })
      return
    }

    // Skip if exists
    if (chat.messages?.some((m) => m.key?.id === message.key?.id)) {
      return
    }
  ...
  const handleMessagesUpdateEvent = (payload) => {
    ...
    let message = payload.data?.messages?.[0] || {}
    let remoteJid = message.key?.remoteJid
    let remoteJidAlt = message.key?.remoteJidAlt

    // check if remoteJid contains @lid then find on remoteJidAlt if contains @s.whatsapp.net, if found then use that
    if (remoteJid.includes('@lid') && remoteJidAlt.includes('@s.whatsapp.net')) {
      remoteJid = remoteJidAlt
    }
  ...
  const syncProfile = () => {
    ...
    axios
      .post(apiUrl(`${sessionId}/chats/${jid}/sync`))
      .then((response) => {
      ...
      .catch((error) => {
        ...
        toast.danger('Failed to sync profile')
      })
  }
