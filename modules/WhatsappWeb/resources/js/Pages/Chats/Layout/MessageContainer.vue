...
<template>
  <div
        ...
        <li v-for="(message, index) in activeConversation?.messages ?? []" class="group" :key="index"
          ...
          <div v-if="!message?.message?.reactionMessage" class="flex gap-x-3 group-[.pr]:flex-row-reverse">
            <div v-if="!message?.key?.fromMe" class="avatar avatar-circle avatar-sm shrink-0">
              <img :src="chatStore.getConversationProfilePic(activeConversation)" class="avatar-img"
                alt="profile-img" />
            </div>
    ...
    <div
      ...
      <form v-if="inputMessage.type != 'voice'" @submit.prevent="sendMessage"
        class="flex h-[4.2rem] items-center rounded-primary shadow">
        <textarea ref="messageInputFieldRef" v-model="inputMessage.message" @keyup.down="onArrowDown"
          @keyup.up="onArrowUp" @keydown.tab.prevent="selectQuickReply" @keydown.enter.exact.prevent="sendMessage"
          ...
          placeholder="Type message... enter to send, shift+enter to next line"></textarea>

        <div class="flex items-center gap-x-4 px-4">
          <button type="button" @click="modalStore.open('aiModal')"
          ...
          <button type="button" @click="chatStore.quickReplyModalOpen"
            class="text-slate-500 hover:text-primary-500 focus:text-primary-500 dark:text-slate-400 dark:hover:text-primary-500 dark:focus:text-primary-500">
            <Icon icon="bxs:layer" />
          </button>

          <button type="button" @click="modalStore.open('TemplateMessageModal')"
            class="text-slate-500 hover:text-primary-500 focus:text-primary-500 dark:text-slate-400 dark:hover:text-primary-500 dark:focus:text-primary-500">
            <Icon icon="bxs:grid" />
          </button>

          <button v-if="chatStore.features?.voice_message" type="button" @click="inputMessage.type = 'voice'"
            class="text-slate-500 hover:text-primary-500 focus:text-primary-500 dark:text-slate-400 dark:hover:text-primary-500 dark:focus:text-primary-500">
            <Icon icon="bxs:microphone" />
          </button>
          <div class="relative">
            <button id="btn-upload-media" @click="assetPopup = !assetPopup" type="button"
              class="text-slate-500 hover:text-primary-500 focus:text-primary-500 dark:text-slate-400 dark:hover:text-primary-500 dark:focus:text-primary-500">
              <Icon icon="bx:link-alt" />
            </button>
            <div class="fixed inset-0 bg-transparent" v-if="assetPopup" @click="assetPopup = false"></div>
            <transition name="fade">
              <div v-if="assetPopup" class="absolute bottom-full right-0">
                ...
                <div class="mb-2 space-y-2 rounded-md bg-dark-900 p-3">
                  ...
                  <button @click="() => modalStore.open('locationMessageModal')" type="button"
