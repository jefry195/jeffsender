...
const toggleAutoReply = () => {
  ...
  let conversationJid = chatStore.activeConversation?.id

  // return if conversationJid contains @lid
  if (conversationJid.includes('@lid')) {
    toastComposable.danger('This conversation is not supported for auto reply')
    return
  }
  ...
  form.put(`/user/conversations/${conversationJid}`)
}
</script>
  ...
  <div class="flex items-center justify-between border-b border-b-slate-200 p-3 dark:border-b-dark-900">
    ...
    <div class="flex items-center justify-start gap-x-3">
      ...
      <button type="button" class="avatar avatar-circle" @click="chatStore.toggleRightSidebar">
        <img class="avatar-img relative" :src="chatStore.getConversationProfilePic(chatStore.activeConversation)" />
