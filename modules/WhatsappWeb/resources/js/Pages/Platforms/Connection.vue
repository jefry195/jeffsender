...
const statusTimeout = 10000 // 10 seconds
...
const loadQrCode = () => {
  ...
  axios
    ...
    .then((res) => {
      ...
      platformSessionData.value.qrCode = res.data?.data?.qrcode
...
const loadCode = () => {
  ...
  axios
    ...
    .then((res) => {
      ...
      platformSessionData.value.code = res.data?.data?.code
...
const startStatusPolling = () => {
  ...
  statusIntervalId = setInterval(() => {
    ...
    axios
      ...
      .then((res) => {
        if (['authenticated'].includes(res.data.data?.status)) {
...
onMounted(() => {
  if (props.platform?.status === 'authenticated') {
    startStatusPolling()
  } else {
    loadQrCode()
  }
    ...
    <div class="card card-body shadow-lg transition-all hover:shadow-xl lg:col-span-8">
      ...
      <template v-if="!isDeviceConnected">
        ...
        >
          ...
          >
            <img :src="platformSessionData?.qrCode" alt="qr-code" class="h-full w-full" />
            <div
