    ...
    <div class="mt-4 grid grid-cols-1 place-items-start gap-4 md:grid-cols-12">
      <div class="card card-body md:col-span-9">
        ...
        <form @submit.prevent="handleFormSubmit">
          <div class="grid grid-cols-2 gap-2">
            ...
            <SelectField v-if="form.message_type === 'template'" class="col-span-1" label="Select a Template"
              v-model="form.template_id" placeholder="SELECT" :validationMessage="form.errors?.template_id"
              :options="templates" />

            <MultiSelect :class="form.message_type === 'text' ? 'col-span-full' : 'col-span-1'" label="Keywords"
              v-model="form.keywords" placeholder="enter keywords" :validationMessage="form.errors?.keywords"
              :options="form.keywords" />

            <div class="col-span-3" v-if="form.message_type === 'text'">
              <TextareaField label="Message" placeholder="Enter the message template" v-model="form.message_template"
                :validationMessage="form.errors?.message_template" :attrs="{ rows: 5 }" />
              <ShortCodes v-model="form.message_template" />
            </div>
          </div>
          <div class="mt-2 flex items-end justify-between">
            <SelectField class="max-w-xs" label="Status" v-model="form.status" placeholder="SELECT"
      ...
      <div class="w-full md:col-span-3">
        <div
          ...
          <div class="absolute bottom-3 left-4 w-10/12 rounded-lg bg-white p-1 px-2 dark:bg-dark-700 xl:w-8/12">
            <TemplatePreview v-if="form.message_type == 'template' && selectedTemplate" :template="selectedTemplate" />
